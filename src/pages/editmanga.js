...
const editManga = async (id, title, number, content) => {
        
    try {
          
      const {data: editmanga} = await axios.put(`http://localhost:1234/manga/edit/${id}`, qs.stringify({title, number, content }), {headers: { 'Content-Type': 'application/x-www-form-urlencoded' }},
    );          
...
const Edit  = (props) => {
    ...
    const EditManga = async e => {
        ...
        console.log(id)
        e.preventDefault()
        await editManga(id, value.title, value.number, value.content)  
    ...
    return (
        <Wrapper>
            <Form>
                ...
                <Button type="submit" onClick={EditManga} >
                Add
                </Button>   
            </Form>
        </Wrapper>  
