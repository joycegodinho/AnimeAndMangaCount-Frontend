...
const newAnime = async (title, number, content) => {
        
    try {
          
      const {data: newanime} = await axios.post("http://localhost:1234/anime/new", qs.stringify({title, number, content }), {headers: { 'Content-Type': 'application/x-www-form-urlencoded' }},
    );          
...
const New  = (props) => {
    ...
    const NewAnime = async e => {
        e.preventDefault()
        const newM = await newAnime(value.title, value.number, value.content)  
    ...
    return (
        <Wrapper>
            <Form>
                ...
                <Button type="submit" onClick={NewAnime} >
                Add
                </Button>   
            </Form>
        </Wrapper>  
